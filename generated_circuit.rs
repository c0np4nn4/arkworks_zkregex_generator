...
impl<F: PrimeField> ConstraintSynthesizer<F> for RegexCircuit<F> {
  fn generate_constraints(self, cs: ConstraintSystemRef<F>) -> Result<(), SynthesisError> {
    ...
    // Initialize state variables (5 states)
    // 현재 상태 초기화
    ...
    for (index, current_input) in input_vars.iter().enumerate() {
      ...
      let cond_2 = is_state_1.and(&(current_input.is_eq(&FpVar::constant(F::from(98u64)))?))?;
      ...
      let cond_3 = is_state_2.and(&(current_input.is_eq(&FpVar::constant(F::from(99u64)))?))?;
      next_state = cond_3.select(&FpVar::constant(F::from(3u64)), &next_state)?;
      has_transitioned = has_transitioned.or(&cond_3)?;

      let is_state_3 = current_state.is_eq(&FpVar::constant(F::from(3u64)))?;
      let cond_4 = is_state_3.and(&(current_input.is_eq(&FpVar::constant(F::from(100u64)))?))?;
      next_state = cond_4.select(&FpVar::constant(F::from(4u64)), &next_state)?;
    ...
    is_accepting = is_accepting.or(&current_state.is_eq(&FpVar::constant(F::from(4u64)))?)?;
